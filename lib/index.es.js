import { defineComponent as k, ref as u, onMounted as b, watch as H, nextTick as N, openBlock as g, createElementBlock as h, createElementVNode as V, normalizeClass as E, normalizeStyle as z, Fragment as F, renderList as S, renderSlot as O, createTextVNode as P, toDisplayString as $ } from "vue";
const j = { id: "p" }, A = ["id"], M = {
  name: "DNDList"
}, W = /* @__PURE__ */ k({
  ...M,
  ...
  setup(i, { expose: a, emit: c }) {
    ...
    let d = u(0);
    const f = { from: 0, to: 0 }, I = u(l.value[0]), r = [];
    function L(e) {
      x(), r.some((s, t) => s.x == e.srcElement.offsetLeft && s.y == e.srcElement.offsetTop ? (d.value = t, p.value = t, I.value = l.value[t], f.from = t, !0) : !1);
    }
    const p = u(0);
    let n, m, y;
    b(() => {
    ...
    H(() => p.value, (e) => {
      d.value != e && (v(), l.value.splice(d.value, 1), l.value.splice(e, 0, I.value), d.value = e, f.to = e, N(x));
    });
    function B(e) {
      e.preventDefault();
      for (let s = r.length; s--; s > 0) {
        const t = r[s + 1], w = r[s];
        if (e.x > w.x && e.y > w.y && (o.direction === "row" ? !t || e.x < t.x : !t || e.y < t.y)) {
          p.value = s;
          break;
        }
      }
...
const q = (i, a) => {
  ...
  for (const [o, l] of a)
    c[o] = l;
  return c;
}, G = /* @__PURE__ */ q(W, [["__scopeId", "data-v-627d1507"]]);
let J = [G];
const D = (i) => {
  J.map((a) => {
